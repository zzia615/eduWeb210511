<%@ Page Title="学员信息" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="StuList.aspx.cs" Inherits="StuList" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="content" Runat="Server">
    ...
    <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataKeyNames="code" DataSourceID="SqlDataSource1" AllowPaging="True" CssClass="table">
        <Columns>
            ...
            <asp:BoundField DataField="needs" HeaderText="需求内容" SortExpression="needs" />
            <asp:BoundField DataField="pubDate" HeaderText="发布日期" SortExpression="pubDate" DataFormatString="{0:d}"/>
            <asp:TemplateField ItemStyle-Width="100">
                <ItemTemplate>
                    <asp:HyperLink ID="HyperLink1" runat="server" CssClass="layui-btn" NavigateUrl='<%#"StuOrder.aspx?code="+Eval("code") %>'>预约</asp:HyperLink>
                </ItemTemplate>
            </asp:TemplateField>
        </Columns>
    </asp:GridView>
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:conStr %>" DeleteCommand="DELETE FROM [TeacherInfo] WHERE [code] = @code" InsertCommand="INSERT INTO [TeacherInfo] ([code], [name], [pwd], [sex], [age], [lxfs], [kemu], [photoUrl]) VALUES (@code, @name, @pwd, @sex, @age, @lxfs, @kemu, @photoUrl)" SelectCommand="SELECT * FROM [StuInfo] where needs>''" UpdateCommand="UPDATE [TeacherInfo] SET [name] = @name, [pwd] = @pwd, [sex] = @sex, [age] = @age, [lxfs] = @lxfs, [kemu] = @kemu, [photoUrl] = @photoUrl WHERE [code] = @code">
        <DeleteParameters>
